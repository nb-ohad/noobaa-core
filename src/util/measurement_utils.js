...
// ------------------------------------------------------------------------------
// Utility functions to allow measurement of code paths without introducing
// memory pressure via allocations. The measurement code is almost allocation
// free! The only allocation made is a slot in map for every new tag introduced
// (which mean only for the first measurement for a new tag). Tags are supposed
// to identify a location in code or and operation which mean that all should be
// allocated after enough time, after that the measurement process will be
// allocation free.
//
// Calculating the metrics for the current measuerments and producing a report
// is an allocaiton heavy operation in shouldbe done outside of paths we would
//  like to measure.
//
// IMPORTANT: Any change introduced to this file should continue to uphole
// to statements above
// ------------------------------------------------------------------------------
...
const measurement_pool = new ObjectPool(
    MU_CONCURENT_MEASUREMENTS,
    () => _init_measurement({}),
    _init_measurement
);
const handle_pool = new ObjectPool(
    MU_CONCURENT_ACTIVE_SAMPLES,
    i => i,
    null,
    -1
);
